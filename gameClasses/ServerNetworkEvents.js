var ServerNetworkEvents = {
	...
	_onPlayerEntity: function (data, clientId) {
		if (!ige.server.players[clientId]) {
			...
			console.log('total players ', ige.server.countplayers);
			
			if(ige.server.countplayers==1 || ige.server.countplayers==3){
				playerroom='room2';
				ige.server.tileData=room1;
				ige.server.room='room2';
			}else{
				playerroom='room1';
				//ige.client.joinRoom('room1');
				ige.server.tileData=room2;
				ige.server.room='room1';
			}
			...
			ige.server.players[clientId].room=playerroom;
			
			ige.server.countplayers++;
			
			
			
			console.log('player room', ige.server.players[clientId].room);
